...
def get_rsi_divergences(df, tf, cur_candle_idx=-1):
    ...
    candles_to_compare = df[df[TIME_COLUMN] < cur_base_value_time - pd.Timedelta(minutes=tf.value[0] * 5)]
    ...
    if cur_base_value_rsi <= 50 and cur_rsi_change < 0:
        ...
        for idx, (past_candle_idx, past_candle) in enumerate(candles_to_compare.iterrows()):
            try:
                ...
                seconds = (df_in_period[TIME_COLUMN] - datetime(1970, 1, 1, tzinfo=pytz.timezone("EST"))).dt.total_seconds()
                slope, intercept, r_value, p_value, std_err = stats.linregress(seconds,
                                                                               df_in_period[
                                                                                   BASE_COLUMN])

                if rsi_change >= 1 and base_value_change <= 0 and slope < 0 and pow(r_value, 2) > 0.3:
                    is_bullish = True
    ...
    # In the following block, we check if there is bearish divergence
    elif cur_base_value_rsi >= 50 and 0 < cur_rsi_change:
        ...
        for idx, (past_candle_idx, past_candle) in enumerate(candles_to_compare.iterrows()):
            try:
                ...
                if rsi_change <= -1 and 0 <= base_value_change and slope > 0 and pow(r_value, 2) > 0.3:
                    is_bearish = True
        ...
        for idx_lcl, (past_candle_idx, past_candle) in enumerate(candles_to_compare.iterrows()):
            try:
                ...
                if candles_to_compare.iloc[idx_lcl - 1][RSI_COLUMN] < \
                    ...
                    if not (35 < past_base_value_rsi < 65):
                        continue
                    ...
                    if rsi_change >= 1 and base_value_change < 0 and slope < 0 < slope2 and pow(r_value, 2) > 0.3:
                        is_bearish = True
    ...
    elif 30 < cur_base_value_rsi <= 70 and cur_rsi_change < 0:
        ...
        for idx_lcl, (past_candle_idx, past_candle) in enumerate(candles_to_compare.iterrows()):
            try:
                ...
                if candles_to_compare.iloc[idx_lcl - 1][RSI_COLUMN] > \
                        past_base_value_rsi < \
                        candles_to_compare.iloc[idx_lcl + 1][RSI_COLUMN]:
                    if not (30 < past_base_value_rsi < 70):
                        continue
                    ...
                    df_in_period = df[
                        (past_base_value_time <= df[TIME_COLUMN]) & (df[TIME_COLUMN] <= cur_base_value_time)]
                    seconds = (df_in_period[TIME_COLUMN] - datetime(1970, 1, 1, tzinfo=pytz.reference.Eastern)).dt.total_seconds()
                    slope, intercept, r_value, p_value, std_err = stats.linregress(seconds,
                                                                                   df_in_period[BASE_COLUMN])
                    ...
                    if rsi_change <= -1 and 0 < base_value_change and slope > 0 > slope2 and pow(r_value,
                                                                                                 2) > 0.3:
                        is_bullish = True
